...
export default function Customer360Page() {
  ...
  const fraudCount       = fraudRecords.filter(f => f.fraud_verdict === 'FRAUD' && !(f.hitl_resolved_by && ['approved', 'credit_approved'].includes(f.order_status))).length
            ...
            <div className="table-wrap"><table>
              ...
              <tbody>{fraudRecords.slice(0, 5).map(f => (
                <tr key={f.fraud_id}>
                  ...
                  <td>
                    {(() => {
                      if (['approved', 'credit_approved'].includes(f.order_status) && f.hitl_resolved_by) {
                        return <span className="badge badge-amber">OVERRIDE</span>;
                      }
                      if (f.order_status === 'cancelled' && f.hitl_resolved_by) {
                        return <span className="badge badge-red">REJECTED</span>;
                      }
                      return <span className={`badge ${f.fraud_verdict === 'FRAUD' ? 'badge-red' : 'badge-green'}`}>{f.fraud_verdict}</span>;
                    })()}
                  </td>
                </tr>
              ))}</tbody>
