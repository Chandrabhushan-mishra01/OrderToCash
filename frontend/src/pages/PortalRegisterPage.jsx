...
const inputStyle = {
  ...
  boxSizing: 'border-box', color: 'var(--text-primary)', background: 'var(--bg-800)',
}
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>New Customer Application</h1>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: 0 }}>Complete your KYC to access the B2B ordering portal</p>
            </div>
          </div>
        </div>
        ...
        {inviteToken && (
          <div style={{
            background: 'rgba(59,130,246,0.1)', border: '1.5px solid #bfdbfe', borderRadius: 12,
        ...
        {/* Form card */}
        <div style={{ background: 'var(--bg-800)', borderRadius: 16, padding: 32, boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
          <form onSubmit={handleRequestOtp}>
            ...
            {error && !showOtp && (
              <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid #fecaca', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#dc2626', marginBottom: 16 }}>
                {error}
              </div>
            )}
      ...
      {showOtp && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: 'var(--bg-800)', width: '100%', maxWidth: 400, borderRadius: 16, padding: 32, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.1)' }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 8px', textAlign: 'center' }}>Verify Email</h2>
            ...
            {otpHint && (
              <div style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid #bfdbfe', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#1d4ed8', marginBottom: 16, textAlign: 'center', fontWeight: 600 }}>
                {otpHint}
              </div>
            )}
            ...
            <form onSubmit={handleVerifyAndSubmit}>
              ...
              {error && (
                <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid #fecaca', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#dc2626', marginBottom: 16, textAlign: 'center' }}>
                  {error}
                </div>
              )}
              ...
              <button
                ...
                style={{ width: '100%', padding: 10, background: 'transparent', border: 'none', color: 'var(--text-secondary)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
              >
                Cancel
              </button>
            </form>
