...
function RepeatModal({ order, onClose, onConfirm }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: 'var(--bg-800)', borderRadius: 16, padding: 28, width: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 4px' }}>Repeat Order</h3>
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 20px' }}>Original: {order.order_id} · {order.product_name}</p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 5 }}>Quantity</label>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 5 }}>Delivery Address</label>
            <input value={address} onChange={e => setAddress(e.target.value)}
              style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #e2e8f0', fontSize: 13, boxSizing: 'border-box' }}
            />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 5 }}>Requested Delivery Date (optional)</label>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1.5px solid #e2e8f0', background: 'var(--bg-800)', color: 'var(--text-secondary)', cursor: 'pointer', fontWeight: 600 }}>
            Cancel
          </button>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 4px' }}>My Orders</h1>
          <p style={{ fontSize: 14, color: 'var(--text-secondary)', margin: 0 }}>{orders.length} orders found</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60 }}>
          ...
          <div style={{ color: 'var(--text-muted)', fontSize: 15 }}>No orders yet. Place your first order from the dashboard!</div>
        </div>
      ) : (
            ...
            <div key={order.order_id} style={{
              ...
              <div style={{ flex: 1, minWidth: 180 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>{order.order_id}</span>
                  <StatusBadge status={order.status} />
                </div>
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 2 }}>{order.product_name || order.sku_id}</div>
              ...
              <div style={{ textAlign: 'right', minWidth: 120 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Qty: {order.quantity}</div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  style={{
                    ...
                    background: 'var(--bg-800)', color: '#3b82f6', fontWeight: 600, fontSize: 12, cursor: 'pointer',
                  }}
                >
