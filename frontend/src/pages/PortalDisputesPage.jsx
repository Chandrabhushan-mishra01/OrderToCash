...
function badgeStyle(status) {
  ...
  if (status === 'withdrawn') return { ...base, background: 'var(--bg-700)', color: 'var(--text-secondary)' }
...
function CreateDisputeForm({ invoices = [], orders = [], onCreated }) {
  ...
  const inputStyle = { width: '100%', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', fontSize: 13, outline: 'none', background: 'var(--bg-800)' }
  const labelStyle = { display: 'block', marginBottom: 5, fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }
  ...
  return (
    <div style={{ background: 'var(--bg-800)', border: '1px solid var(--border)', borderRadius: 14, padding: 18, boxShadow: '0 1px 3px rgba(15,23,42,0.06)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        <Plus size={18} color="#2563eb" />
        <h2 style={{ margin: 0, fontSize: 18, color: 'var(--text-primary)' }}>Submit a Dispute</h2>
      </div>

      <div style={{ background: 'var(--bg-900)', border: '1px solid var(--border)', borderRadius: 12, padding: 14, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <Bot size={16} color="#6366f1" />
          <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--text-primary)' }}>Describe the dispute in your own words</div>
        </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ marginTop: 6, fontSize: 12, color: 'var(--text-secondary)' }}>Allowed: PDF, PNG, JPG, JPEG. Max 5 files. AI does not submit anything until you click Submit.</div>
      </div>
      <button
...
function DisputeThread({ disputeId, onBack }) {
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--bg-800)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 18, background: 'var(--bg-900)', minHeight: 260 }}>
          {messages.map((m) => {
            const isCustomer = m.sender_type === 'customer'
            const isSystem = m.sender_type === 'system'
            return (
              <div key={m.message_id} style={{ display: 'flex', justifyContent: isCustomer ? 'flex-end' : 'flex-start', marginBottom: 12 }}>
                <div style={{ maxWidth: '74%', background: isSystem ? '#f1f5f9' : isCustomer ? '#2563eb' : 'white', color: isCustomer ? 'white' : '#0f172a', border: isSystem ? '1px dashed #cbd5e1' : '1px solid var(--border)', borderRadius: 14, padding: '10px 12px', boxShadow: '0 1px 2px rgba(15,23,42,0.05)' }}>
        ...
        <div style={{ padding: 18, borderTop: '1px solid var(--border)' }}>
          {!canReply && !FINAL_STATUSES.includes(dispute.status) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(245,158,11,0.1)', color: '#92400e', padding: 11, borderRadius: 9, fontSize: 13, marginBottom: 12 }}>
              <AlertCircle size={15} /> Waiting for Admin response. You can reply after Admin responds.
            </div>
          )}
          {FINAL_STATUSES.includes(dispute.status) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-700)', color: 'var(--text-secondary)', padding: 11, borderRadius: 9, fontSize: 13, marginBottom: 12 }}>
              <CheckCircle size={15} /> This dispute is final and cannot receive new messages.
            </div>
          )}
          <textarea rows={4} disabled={!canReply} value={message} onChange={(e) => setMessage(e.target.value)} placeholder={canReply ? 'Write your response to Admin...' : 'Reply is locked until Admin responds'} style={{ width: '100%', border: '1px solid var(--border)', borderRadius: 9, padding: 12, resize: 'vertical', opacity: canReply ? 1 : 0.55 }} />
      ...
      {showWithdraw && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 50 }}>
          <div style={{ width: '100%', maxWidth: 460, background: 'var(--bg-800)', borderRadius: 14, padding: 18 }}>
            <h3 style={{ marginTop: 0 }}>Withdraw dispute?</h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>This will close the dispute from your side. Admin will still see it for audit history.</p>
            <textarea rows={4} value={withdrawReason} onChange={(e) => setWithdrawReason(e.target.value)} placeholder="Optional reason" style={{ width: '100%', border: '1px solid var(--border)', borderRadius: 9, padding: 10 }} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button onClick={() => setShowWithdraw(false)} style={{ border: '1px solid var(--border)', background: 'var(--bg-800)', borderRadius: 8, padding: '8px 12px', cursor: 'pointer' }}>Cancel</button>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 28, color: 'var(--text-primary)' }}>Disputes</h1>
          <p style={{ margin: '6px 0 0', color: 'var(--text-secondary)', fontSize: 14 }}>Submit disputes, upload proof, and chat with Admin in controlled turns.</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => refetch()} style={{ display: 'inline-flex', gap: 6, alignItems: 'center', border: '1px solid var(--border)', background: 'var(--bg-800)', borderRadius: 8, padding: '9px 12px', cursor: 'pointer' }}><RefreshCcw size={14} /> Refresh</button>
      ...
      <div style={{ background: 'var(--bg-800)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: 16, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ color: 'var(--text-secondary)', fontSize: 13 }}>({disputes.length})</span>
        </div>
        {isLoading ? (
          ...
          <div style={{ padding: 34, textAlign: 'center', color: 'var(--text-secondary)' }}>
            ...
            <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>No disputes yet</div>
            <div style={{ fontSize: 13 }}>Create a dispute when you need Admin review for an invoice or order.</div>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead style={{ background: 'var(--bg-900)', color: 'var(--text-secondary)', textAlign: 'left' }}>
                <tr>
              ...
              <tbody>
                {disputes.map((d) => (
                  <tr key={d.dispute_id} style={{ borderTop: '1px solid var(--border)' }}>
                    <td style={{ padding: 12, fontFamily: 'monospace', fontSize: 12 }}>{d.dispute_id}</td>
                    <td style={{ padding: 12 }}>
                      <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{d.subject}</div>
                      <div style={{ fontSize: 12, color: 'var(--text-secondary)', textTransform: 'capitalize' }}>{(d.dispute_type || '').replace(/_/g, ' ')} · Proofs: {d.proof_count || 0}</div>
                    </td>
                    ...
                    <td style={{ padding: 12, color: 'var(--text-secondary)' }}>{formatDate(d.updated_at)}</td>
                    <td style={{ padding: 12 }}><button onClick={() => setSelectedId(d.dispute_id)} style={{ border: '1px solid var(--border)', background: 'var(--bg-800)', borderRadius: 8, padding: '7px 10px', cursor: 'pointer', fontWeight: 700 }}>Open</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
