...
async def lifespan(app: FastAPI):
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT o.total_amount_inr, o.quantity, o.unit_price_inr, o.channel,
                          c.avg_dso_days, c.missed_payments_12m, c.open_ar_balance_inr, c.account_age_months, c.credit_limit_inr
                   FROM orders o
    ...
    # Start Auto-Dunning Scheduler — runs once every 24 hours
    # Sends automated reminder emails for overdue invoices using the same
    # dunning_log, segmentation, and Groq generation as the manual Collections panel.
    async def _auto_dunning_loop():
        """
        Agent 8 (Auto) — Automated Reminder Service.
        Scans overdue invoices every AUTO_DUNNING_INTERVAL_MINUTES (default 60 min).
        Cooldown rules inside auto_dunning.py prevent duplicate emails per invoice+level.
        Logs all sends to dunning_log with triggered_by='auto_scheduler'.
        """
        await asyncio.sleep(60)   # 60-sec delay — let DB pool + ML models fully warm up
        while True:
            try:
                from database.postgres import get_pool as _get_pool
                from services.auto_dunning import run_auto_dunning_cycle
                from config import settings as _s
                pool = await _get_pool()
                summary = await run_auto_dunning_cycle(pool)
                logger.info(
                    "Auto-dunning scan: sent=%d flagged=%d skipped=%d errors=%d",
                    summary.get("sent", 0),
                    summary.get("flagged_for_staff", 0),
                    summary.get("skipped_cooldown", 0),
                    summary.get("errors", 0),
                )
            except Exception as exc:
                logger.error("Auto-dunning scheduler error: %s", exc)
            await asyncio.sleep(settings.auto_dunning_interval_minutes * 60)

    asyncio.create_task(_auto_dunning_loop())
    logger.info("Auto-dunning scheduler started (24-hour interval)")
...
app.include_router(chat_router,            prefix="/api/chat",            tags=["Internal Chat"])
