...
Tone guide:
- gentle_reminder: Polite, relationship-first. Assume the customer simply forgot. Warm opening, no threats.
- firm_premium: Firm but highly respectful. Acknowledge the long-standing partnership. Express genuine concern.
  Mention that continued delay may affect credit terms — but frame it as wanting to resolve together, not a threat.
- firm: Standard overdue notice. Professional and direct. Mention potential service hold if unpaid in 7 days.
- urgent_premium: Serious escalation for a valued customer. No legal threats. Request escalation to their
  CFO or Finance Head. Mention a credit limit review is under consideration. Tone is professional and urgent,
  not harsh. Preserve the relationship while making the urgency unmistakably clear.
- urgent: Escalated tone. Offer a payment plan. Clearly state consequences (service suspension, credit hold).
  Firm but still professional.
- legal_warning_soft: Strong final warning. Formal language. State that the account will be referred to a
  formal collections process within 5 business days unless full payment is received. Avoid naming a specific
  legal firm. Slightly more recoverable than a full legal warning.
- legal_warning: Final notice. Unambiguous. Clearly state handoff to collections/legal in 48 hours.
  No further extensions will be granted. Use formal legal language.

Always include: Invoice ID, amount due, days overdue, and a clear call to action with a specific deadline.
...
def resolve_dunning_tone(segment: str, days_overdue: int) -> str:
    """
    Resolve the exact Groq tone from customer segment × dunning level.

    Tone matrix (segment × level):
    ...
    matrix = {
        "Premium":  {1: "gentle_reminder",  2: "firm_premium",       3: "urgent_premium"},
        "Standard": {1: "gentle_reminder",  2: "firm",               3: "urgent"},
        "At-Risk":  {1: "firm",             2: "urgent",             3: "legal_warning_soft"},
        "Problem":  {1: "urgent",           2: "legal_warning_soft", 3: "legal_warning"},
    }
    return matrix.get(segment, matrix["Standard"])[level]
...
def generate_dunning_email(
    ...
    contact_name: str = "",
    tone: str = None,          # explicit tone overrides segment-based resolution
    segment: str = "Standard", # customer k-means segment — used when tone is None
) -> Dict[str, str]:
    """Generate a personalised dunning email using Groq LLaMA 3.3 70B.

    Tone resolution (Option A):
      - If *tone* is explicitly passed, it is used as-is (backward compatible).
      - If *tone* is None, the tone is resolved from the segment × days_overdue matrix
        via resolve_dunning_tone().  This ensures automated and manual callers get the
        most contextually appropriate tone without each caller implementing the matrix.

    Segment values: Premium | Standard | At-Risk | Problem
    """
    level = 1 if days_overdue <= 15 else (2 if days_overdue <= 30 else 3)
    resolved_tone = tone if tone is not None else resolve_dunning_tone(segment, days_overdue)
    messages = [
    ...
    try:
        raw = _call_groq(messages, json_mode=True)
        result = json.loads(raw)
        logger.info(f"Groq dunning L{level} [{resolved_tone}] segment={segment} generated for {invoice_id} ({days_overdue}d overdue)")
        return result
    except Exception as e:
