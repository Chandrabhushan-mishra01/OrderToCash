...
def _order_features(order: Dict[str, Any]) -> List[float]:
    """
    Extract relative numerical features for a single order to prevent
    'bulk' vs 'small' customer bias in the global Isolation Forest.
    """
    amount = float(order.get("total_amount_inr", 0))
    qty = float(order.get("quantity", 1))
    
    # Customer-specific baselines
    credit_limit = float(order.get("credit_limit_inr", 100000))
    if credit_limit <= 0:
        credit_limit = 100000  # Safe fallback to prevent div/0
    
    avg_dso = float(order.get("avg_dso_days", 30))
    open_ar = float(order.get("open_ar_balance_inr", 0))
    
    # Relative features: ratio of this order and current balance vs credit limit
    credit_utilization_ratio = amount / credit_limit
    open_ar_ratio = open_ar / credit_limit
    
    unit_price = float(order.get("unit_price_inr", 0))

    return [
        credit_utilization_ratio,                    # 1: Order size relative to credit limit
        open_ar_ratio,                               # 2: Current outstanding AR relative to credit limit
        unit_price,                                  # 3: Raw unit price (product profile)
        avg_dso,                                     # 4: Customer's typical payment speed
        float(order.get("missed_payments_12m", 0)),  # 5: Reliability
        float(order.get("account_age_months", 12)),  # 6: Customer tenure
