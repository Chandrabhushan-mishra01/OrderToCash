...
SEARCH_QUERY = f"-label:{config.PROCESSED_LABEL} in:inbox is:unread"
...
class GmailClient:
    ...
    def _get_or_create_label_id(self) -> str:
        ...
        for label in existing.get("labels", []):
            if label["name"].lower() == config.PROCESSED_LABEL.lower():
        ...
        try:
            ...
            if e.resp.status == 409:
                # Label already exists but wasn't caught (e.g. case conflict). Re-fetch.
