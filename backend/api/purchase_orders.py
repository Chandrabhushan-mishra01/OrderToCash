...
PO_READ_ROLES = ["admin", "inventory_manager"]
PO_WRITE_ROLES = ["admin", "inventory_manager"]
...
class PurchaseOrderLineCreate(BaseModel):
    ...
    suggested_selling_price_inr: Optional[float] = None
...
class ReceiveLine(BaseModel):
    ...
    new_unit_price_inr: Optional[float] = None       # purchase cost from this PO
    new_selling_price_inr: Optional[float] = None    # selling price to set (admin-decided)
...
async def nlp_po_preview(
    payload: NLPPORequest,
    staff=Depends(require_role(PO_WRITE_ROLES)),
):
    """
    Use GLiNER + Groq to extract PO fields from admin free-text.
    Returns a pre-filled form payload for review before creating the PO.
    Example input: 'Order 100 units of SKU-001 and 50 of SKU-004 from Motorco, expected by 2026-07-15'
    """
    from ml.gliner_ner import extract_po_entities
    result = extract_po_entities(payload.text)
    groq = result.get("groq", {})
    confidence = result.get("_confidence", "MEDIUM")

    items = groq.get("items") or []
    # Ensure items have the right keys and sensible defaults
    clean_items = []
    for item in items:
        purchase_cost = float(item.get("unit_cost_inr") or 0)

        # Priority:
        #   1. Admin explicitly stated a selling price in text → use it directly
        #   2. Fallback: purchase_cost × 1.10 as a suggestion
        explicit_selling = item.get("selling_price_inr")
        if explicit_selling and float(explicit_selling) > 0:
            suggested_selling = float(explicit_selling)
            selling_source = "explicit"      # admin stated it directly
        elif purchase_cost > 0:
            suggested_selling = round(purchase_cost * 1.10, 2)
            selling_source = "suggested_10pct"   # AI computed +10%
        else:
            suggested_selling = 0
            selling_source = "none"

        clean_items.append({
            "sku_id": str(item.get("sku_id", "")).upper().strip(),
            "quantity_ordered": max(1, int(item.get("quantity_ordered") or 1)),
            "unit_cost_inr": purchase_cost,
            "suggested_selling_price_inr": suggested_selling,
            "selling_price_source": selling_source,  # "explicit" | "suggested_10pct" | "none"
        })

    # If no items extracted, return one empty placeholder
    if not clean_items:
        clean_items = [{"sku_id": "", "quantity_ordered": 1, "unit_cost_inr": 0, "suggested_selling_price_inr": 0, "selling_price_source": "none"}]

    return {
        "prefilled": {
            "supplier_id": (groq.get("supplier_id") or "").strip(),
            "expected_arrival_date": groq.get("expected_arrival_date") or "",
            "items": clean_items,
        },
        "confidence": confidence,
        "message": "Review and adjust the extracted fields before creating the PO.",
        "raw": groq,
    }
                ...
                await db.execute(
                    """INSERT INTO purchase_order_items
                       (po_item_id, po_id, sku_id, quantity_ordered, quantity_received,
                        unit_cost_inr, suggested_selling_price_inr, line_status)
                       VALUES ($1,$2,$3,$4,0,$5,$6,'open')""",
                    _po_item_id(), po_id, item.sku_id, item.quantity_ordered,
                    item.unit_cost_inr, item.suggested_selling_price_inr,
                )
