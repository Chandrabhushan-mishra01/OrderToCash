...
async def submit_dispute_email(payload: DisputeEmailIngest, db=Depends(get_db)):
    ...
    try:
        async with db.transaction():
            ...
            # 1. If NER extracted an Order ID as the invoice ref, shift it
            if invoice_ref and invoice_ref.startswith("ORD-"):
                if not order_ref:
                    order_ref = invoice_ref
                invoice_ref = "UNKNOWN"

            # 2. Try to resolve Order ID
            if order_ref:
                ...
                if order_row:
                    final_order_id = order_row["order_id"]

            # 3. Try to resolve Invoice ID
            if invoice_ref and invoice_ref != "UNKNOWN":
                ...
                if invoice_row:
                    final_invoice_id = invoice_row["invoice_id"]
                    if not final_order_id:
                        final_order_id = invoice_row["order_id"]
            
            # 4. If we have an order_id but NO invoice_id, look up the invoice for that order
            if final_order_id and not final_invoice_id:
                inv_row = await db.fetchrow(
                    "SELECT invoice_id FROM invoices WHERE order_id=$1 AND customer_id=$2 LIMIT 1",
                    final_order_id, customer_id
                )
                if inv_row:
                    final_invoice_id = inv_row["invoice_id"]
            ...
            await db.execute(
                ...
                dispute_id, customer_id, final_invoice_id,
                final_order_id, dispute_type, subject,
            ...
            await db.execute(
                ...
                customer_id, final_invoice_id,
                json.dumps({"dispute_id": dispute_id, "dispute_type": dispute_type, "severity": severity, "order_id": final_order_id})
            )
            ...
            if payload.invoice_id:
                try:
                    ...
                    await apply_credit_memo_to_invoice(
                        ...
                        memo_amount = float(payload.approved_amount),
                    )
