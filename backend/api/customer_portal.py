...
def send_email(
    ...
    try:
        ...
        logger.info(f"Email sent to {to}: {subject} (attachments: {len(attachments or [])})")
    except Exception as e:
        logger.error(f"Email send failed to {to}: {e}")


def send_email_with_attachment(to: str, subject: str, body: str, attachment_path: str | None = None, attachment_name: str | None = None):
    """
    Send email via Gmail SMTP with an optional file attachment.
    Falls back to plain-text send if attachment is missing or unreadable.
    Silently logs on SMTP failure so callers are never broken.
    """
    if not settings.smtp_user or not settings.smtp_password:
        logger.warning("SMTP not configured — email not sent")
        return
    try:
        ...
        if attachment_path and os.path.isfile(attachment_path):
            ...
            logger.warning(f"Attachment not found, sending without it: {attachment_path}")
...
async def pay_invoice_mock(
    ...
    # Link memo to invoice, recalculate balance, append audit event
    try:
        from api.invoices import apply_credit_memo_to_invoice
        await apply_credit_memo_to_invoice(
            db          = db,
            memo_id     = memo_id,
        ...
        logger.error(f"apply_credit_memo_to_invoice failed for {memo_id}: {link_err}")
...
async def place_order(
    ...
    if status == "approved":
        ...
        seller_gstin = _os.environ.get("MOCK_SELLER_GSTIN", "27AABCM1234F1ZY")
        cust_gstin = customer.get("gstin") or customer.get("gst_no") or ""
        irp = generate_mock_irp_data(
        ...
        await db.execute(
            """INSERT INTO invoices (invoice_id, order_id, customer_id, due_date,
               subtotal_inr, gst_amount_inr, total_amount_inr, balance_due_inr,
               payment_status, payment_token,
               irn, ack_no, ack_date, signed_qr_code, eway_bill_no)
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'pending', $9,
                       $10, $11, $12, $13, $14)""",
            invoice_id, order_id, customer["customer_id"], due_date,
            subtotal, gst_amount, total, total, payment_token,
            irp["irn"], irp["ack_no"], irp["ack_date"],
            irp["signed_qr_code"], irp["eway_bill_no"],
        )
        await db.execute(
        ...
        await db.execute(
            "INSERT INTO audit_log (event_type, agent_name, order_id, action, details) VALUES ($1,$2,$3,$4,$5)",
            "INVOICE_GENERATED", "agent_05_fulfillment", order_id, "generate_invoice",
            json.dumps({"output": {"invoice_id": invoice_id, "status": "Auto-generated", "irn": irp["irn"]}})
        )
        logger.info(f"Agent 5: invoice {invoice_id} with IRN {irp['irn'][:20]}… for portal order {order_id}")

        # Generate PDF — wrapped so failure never blocks order
        try:
            invoice_row = {
                "invoice_id": invoice_id, "invoice_date": datetime.utcnow(),
                "due_date": due_date, "subtotal_inr": subtotal,
                "gst_amount_inr": gst_amount, "total_amount_inr": total,
                "balance_due_inr": total, "payment_status": "pending",
                "payment_token": payment_token, **irp,
            }
            order_row = {
                "order_id": order_id, "sku_id": payload.sku_id,
                "quantity": payload.quantity,
                "unit_price_inr": unit_price, "gst_pct": gst_pct,
            }
            ...
            await db.execute(
                "UPDATE invoices SET invoice_pdf_path = $1 WHERE invoice_id = $2",
                invoice_pdf_path, invoice_id,
            )
            logger.info(f"Portal invoice PDF generated: {invoice_pdf_path}")
        except Exception as pdf_err:
            logger.error(f"Portal invoice PDF generation failed for {invoice_id}: {pdf_err}")

    # Send confirmation email — attach PDF if generated
    if status == "approved" and payment_token:
        ...
        try:
            send_email(
                to=customer["email"],
                subject=f"Invoice {invoice_id} — ₹{total:,.0f} | MAQ Manufacturing",
                body=(
            ...
            _email_err = str(_e)
            logger.error(f"Portal invoice email failed for {invoice_id}: {_e}")

        # Log send attempt — sent_at updated by log_invoice_email on success
        try:
            ...
            await log_invoice_email(
                ...
                status     = "failed" if _email_err else "sent",
                error      = _email_err,
            )
        except Exception as log_err:
            logger.error(f"invoice_email_log insert failed for {invoice_id}: {log_err}")
    else:
        send_email(
            to=customer["email"],
...
async def repeat_order(
    ...
    async with db.transaction():
        ...
        # ── Auto-generate Invoice if approved (mirrors place_order flow) ──
        payment_token = None
        invoice_id = None
        if status == "approved":
            import random as _random
            invoice_id = f"INV-{new_order_id.split('-')[1]}"
            due_date = datetime.utcnow() + timedelta(days=30)
            payment_token = f"{_random.randint(0, 999_999_999_999):012d}"
            await db.execute(
                """INSERT INTO invoices (invoice_id, order_id, customer_id, due_date,
                ...
                invoice_id, new_order_id, customer["customer_id"], due_date,
            ...
            await db.execute(
                "INSERT INTO audit_log (event_type, agent_name, order_id, action, details) VALUES ($1,$2,$3,$4,$5)",
                "INVOICE_GENERATED", "agent_05_fulfillment", new_order_id, "generate_invoice",
                json.dumps({"output": {"invoice_id": invoice_id, "status": "Auto-generated from repeat order"}})
            )
            logger.info(f"Repeat order {new_order_id}: Auto-generated invoice {invoice_id}")

    # Send confirmation email if approved
    if status == "approved" and invoice_id and payment_token:
        pay_link = f"{settings.frontend_url}/portal/outstanding"
        send_email(
            to=customer["email"],
            subject=f"Invoice Generated — {invoice_id} | MAQ Manufacturing",
            body=(
                f"Dear {customer.get('contact_name', customer.get('company_name', 'Customer'))},\n\n"
                f"Your repeat order has been approved and an invoice has been raised.\n\n"
                f"New Order ID   : {new_order_id}\n"
                f"Original Order : {order_id}\n"
                f"Invoice ID     : {invoice_id}\n"
                f"Quantity       : {qty}\n"
                f"Total (GST)    : ₹{total:,.0f}\n"
                f"Due Date       : {due_date.strftime('%d %b %Y')}\n\n"
                f"To pay, visit: {pay_link}\n\n"
                f"Regards,\nMAQ Manufacturing — Finance Team"
            )
        )
    ...
    return {
        ...
        "unit_price_inr": unit_price,
        "total_amount_inr": total,
        "message": f"Order repeated successfully from {order_id}." + (f" Invoice {invoice_id} raised." if invoice_id else ""),
        **_inventory_response_fields(inventory_result),
    }
...
# ══════════════════════════════════════════════════════════════════════════
# INVOICE PDF DOWNLOAD (customer-owned invoices only)
# ══════════════════════════════════════════════════════════════════════════

# Canonical PDF directory — used for path-traversal guard
_PORTAL_PDF_DIR = (Path(__file__).parent.parent / "invoice_pdfs").resolve()


def _portal_safe_pdf_path(raw_path: str) -> Path:
    """
    Resolve *raw_path* and confirm it stays inside _PORTAL_PDF_DIR.
    Raises HTTP 403 if the path escapes the allowed directory.
    """
    resolved = Path(raw_path).resolve()
    try:
        resolved.relative_to(_PORTAL_PDF_DIR)
    except ValueError:
...
async def download_my_invoice_pdf(
    ...
    """
    Download a PDF for an invoice that belongs to the authenticated customer.
    Returns 404 if the invoice does not exist.
    Returns 403 if the invoice belongs to a different customer.
    Regenerates the PDF on-the-fly if it has not been generated yet.
    """
    inv = await db.fetchrow(
    ...
    # Ownership check — must belong to the authenticated customer
    if inv.get("customer_id") != customer["customer_id"]:
        raise HTTPException(
            status_code=403,
            detail="You are not authorised to access this invoice.",
        )
    ...
    if pdf_path:
        try:
            safe = _portal_safe_pdf_path(pdf_path)
            if safe.is_file():
                return FileResponse(
                    path       = str(safe),
            ...
            logger.warning(
                "Portal: cached PDF unreadable for %s (%s) — regenerating", invoice_id, exc
            )

    # ── Regenerate PDF ───────────────────────────────────────────────────
    logger.info("Portal: regenerating invoice PDF for %s", invoice_id)
    from services.invoice_pdf import generate_invoice_pdf

    order_id = inv.get("order_id")
    order = (
        await db.fetchrow("SELECT * FROM orders WHERE order_id = $1", order_id)
    ...
    try:
        ...
        logger.error("Portal: PDF regeneration failed for %s: %s", invoice_id, exc)
        raise HTTPException(
            status_code=500,
            detail=f"Could not generate invoice PDF: {exc}",
        )
    ...
    safe_new = _portal_safe_pdf_path(new_path)
    return FileResponse(
        path       = str(safe_new),
        media_type = "application/pdf",
        filename   = f"invoice-{invoice_id}.pdf",
