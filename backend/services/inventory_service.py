...
async def receive_purchase_order(
    ...
    async with db.transaction():
        ...
        for line in received_items:
            ...
            product = await db.fetchrow(
                "SELECT stock_on_hand, incoming_stock, base_price_inr, cost_price_inr FROM products WHERE sku_id = $1 FOR UPDATE",
                sku_id,
            )
            if not product:
                raise ValueError(f"SKU {sku_id} not found")
            ...
            # ── Selling Price & Cost Price update ────────────────────────────
            new_selling_price = line.get("new_selling_price_inr")
            new_unit_price = line.get("new_unit_price_inr")   # purchase cost
            old_selling = float(product["base_price_inr"]) if product["base_price_inr"] else None
            price_updated = False

            if new_selling_price and float(new_selling_price) > 0:
                new_selling_price = float(new_selling_price)
                cost_to_store = float(new_unit_price) if new_unit_price and float(new_unit_price) > 0 else None
                if cost_to_store is not None:
                    await db.execute(
                        "UPDATE products SET incoming_stock=$1, stock_on_hand=$2, base_price_inr=$3, cost_price_inr=$4, updated_at=NOW() WHERE sku_id=$5",
                        new_incoming, new_stock, new_selling_price, cost_to_store, sku_id,
                    )
                else:
                    await db.execute(
                        "UPDATE products SET incoming_stock=$1, stock_on_hand=$2, base_price_inr=$3, updated_at=NOW() WHERE sku_id=$4",
                        new_incoming, new_stock, new_selling_price, sku_id,
                    )
                price_updated = True
            else:
                # No selling price update — still update cost_price_inr if provided
                new_unit_price_val = float(new_unit_price) if new_unit_price and float(new_unit_price) > 0 else None
                if new_unit_price_val:
                    await db.execute(
                        "UPDATE products SET incoming_stock=$1, stock_on_hand=$2, cost_price_inr=$3, updated_at=NOW() WHERE sku_id=$4",
                        new_incoming, new_stock, new_unit_price_val, sku_id,
                    )
                else:
                    await db.execute(
            ...
            # Log price revaluation if selling price was updated
            if price_updated and old_selling is not None:
                price_meta = json.dumps({
                    "old_selling_price": old_selling,
                    "new_selling_price": new_selling_price,
                    "purchase_cost": float(new_unit_price) if new_unit_price else None,
                    "po_id": po_id,
                })
                await db.execute(
                    """INSERT INTO inventory_transactions
                       (txn_id, sku_id, txn_type, quantity_delta, field_affected,
                        balance_after, purchase_order_id, reason, performed_by, actor_type, created_at, metadata)
                       VALUES ($1,$2,'PRICE_REVALUATION',0,'base_price_inr',$3,$4,$5,$6,$7,$8,$9)""",
                    _txn_id(), sku_id, new_selling_price, po_id,
                    f"Selling price updated via PO {po_id}: ₹{old_selling:,.0f} → ₹{new_selling_price:,.0f}",
                    performed_by, actor_type, now, price_meta,
                )
            ...
            movements.append({
                ...
                "new_selling_price_inr": new_selling_price if price_updated else None,
            })
